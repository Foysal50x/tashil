...
Need guaranteed-unique ids? Have your generator implement `Foysal50x\Tashil\Contracts\ShouldBeUnique` and add an `isUnique(string $id): bool` check — `generate()` re-renders until it passes, then throws `UniqueIdGenerationException` if the format space is exhausted. Generators that don't implement it return the rendered id as-is. See [docs/06-Developer-Guide.md › Guaranteed-unique ids](docs/06-Developer-Guide.md#guaranteed-unique-ids-opt-in).
